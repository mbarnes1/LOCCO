...
methods = {'T4+mono', 'Basis', 'Sketch', 'True'};
method_counter = 1;
...
if any(strcmp(methods, 'Medoid'))
    s_sketch_medoid = filter_sketch(A, b, sketch_medoid, 'medoid', 0, false);
...
if any(strcmp(methods, 'Block'))
    s_sketch_block = filter_sketch(A, b, sketch_block, 'block', lambda, true);
...
if or(any(strcmp(methods, 'Mean')), any(strcmp(methods, 'Sketch')))
    s_sketch_mean = filter_sketch(A, b, sketch_mean, 'mean', lambda, true);
